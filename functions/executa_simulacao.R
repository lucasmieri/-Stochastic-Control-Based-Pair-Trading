...
execute_simulations <- function(pairs, init_date_treino, end_date_treino, valor_inicial) {
  ...
  for (i in seq_along(pairs)) {
    ...
    warning = function(w) {
      message("Warning occurred with pair: ", paste0(pair, collapse = " "), ". Details: ", w$message)
    }, 
    error = function(e) {
      message("An error occurred with pair: ", paste0(pair, collapse = " "), ". Details: ", e$message, ". Skipping this pair.")

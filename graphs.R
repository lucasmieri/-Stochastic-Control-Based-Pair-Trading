...
results <- readRDS(file = "simulation_results.rds")
...
create_save_plot <- function(result, params, pair) {
  ...
  # Save the plot
  ggsave(filename = paste0("F:/l_mie/Documents/new_version/graphs/", pair[1], "_", pair[2], ".png"), plot = p)
...
create_distributions <- function(results) {
  ...
  ggsave(filename = "F:/l_mie/Documents/new_version/graphs/operational_return_distribution.png", plot = p1)
  ...
  ggsave(filename = "F:/l_mie/Documents/new_version/graphs/holding_days_distribution.png", plot = p2)
}
...
# Use the function to create the distributions
create_metrics_distributions(metrics, "F:/l_mie/Documents/new_version/graphs")
...
create_metrics_plot <- function(df, pair) {
  ...
  # Save the plot
  ggsave(filename = paste0("F:/l_mie/Documents/new_version/graphs/", "Metrics_",pair[1], "_", pair[2], ".png"), plot = p)
}
...
create_metrics_distribution <- function(results) {
  # Combine all df_final
  all_df_final <- bind_rows(lapply(results, function(x) x$df_final))
  
  # Summarize data
  summarized_data <- all_df_final %>%
    group_by(pair_id) %>%  # Replace 'pair_id' with the correct column name
    summarize(mean_value = mean(value_column), .groups = 'drop')  # Replace 'value_column' with the column you want to summarize
  
  # Combine params
  all_params <- bind_rows(lapply(results, function(x) x$params))
  
  # Join the summarized data with params
  metrics <- inner_join(summarized_data, all_params, by = "pair_id")  # Replace 'pair_id' with the correct column name
...
# Use the function to create the distributions
create_metrics_distribution(results)
...
# Use the function to get the summary statistics
summary_stats(metrics)
>>>>>>> a4ddfcc97178911a7a127a8255eb1d735999df6d
